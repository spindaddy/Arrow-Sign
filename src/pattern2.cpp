...
constexpr int HEAD_LEDS = 2;    // 2 LEDs per group
...
// 50% of Pattern 1 speed → 2x the sync time
constexpr uint32_t SYNC_B_MS = 1334;
constexpr float P_A =
...
void pattern2Setup(Adafruit_NeoPixel &stripShort, Adafruit_NeoPixel &stripLong) {
  ...
  (void)stripLong;
  chaseStartMs = millis();
  Serial.println("  Chase Half: same as Chase Fast, 50% speed");
}
